...
function SignUpForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (
      !form.email.endsWith("@oouagoiwoye.edu.ng") &&
      !form.email.endsWith("@student.oouagoiwoye.edu.ng")
    ) {
  ...
  return (
    ...
    >
      ...
      <div className="flex bg-gray-100 rounded-full p-1 mb-8 text-sm font-medium">
        ...
        <span className="flex-1 text-center py-2 rounded-full bg-white shadow-sm">
          Sign up
        </span>
      </div>
      ...
      <label className="text-sm font-medium block mb-2" htmlFor="signup-email">
        Email
      </label>
      <input
        id="signup-email"
      ...
      {role === "student" && (
        <>
          <label className="text-sm font-medium block mb-2" htmlFor="level">
            Level
          </label>
          <select
            id="level"
    ...
    <div className="min-h-screen grid grid-cols-1 md:grid-cols-2">
      ...
      <div className="relative bg-[#0a0a0a] text-white px-10 md:px-16 py-16 flex flex-col justify-between overflow-hidden">
        ...
        <div className="relative flex items-center gap-3">
          ...
          />
          <span className="font-medium">Campus Connect Portal</span>
        </div>
        ...
        >
          ...
          <p className="text-white/55 leading-relaxed">
            One home for schedules, attendance, assignments, complaints and the
            latest news from the Department of Computer Engineering.
          </p>
        </motion.div>

        <p className="relative text-xs text-white/30">
          © 2026 Olabisi Onabanjo University
        </p>
      </div>
