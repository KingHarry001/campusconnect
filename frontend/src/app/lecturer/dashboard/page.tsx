      ...
      <div className="grid md:grid-cols-2 gap-4">
        <div className="rounded-3xl border border-gray-100 p-6 bg-green-50">
          <h3 className="font-medium text-green-900 mb-1">
            Tip: Open an attendance session
          </h3>
          <p className="text-sm text-green-700 leading-relaxed">
            Navigate to <strong>Attendance Sessions</strong> to open a timed
            check-in window for any class. Students must be within the set GPS
        ...
        <div className="rounded-3xl border border-gray-100 p-6 bg-amber-50">
          ...
          <p className="text-sm text-amber-700 leading-relaxed">
            You have {stats.complaints} open complaint
            {stats.complaints !== 1 ? "s" : ""} awaiting your response. Visit
            the <strong>Complaints</strong> tab to reply.
          </p>
        </div>
      </div>
    </div>
    ...
    const load = async () => {
      ...
      courses?.forEach((c: any) => {
        c.classes?.forEach((cls: any) =>
          allClasses.push({ ...cls, courseCode: c.code, courseTitle: c.title }),
        );
      });
          ...
          >
            ...
            {classes.map((c) => (
              <option key={c.id} value={c.id}>
                {c.courseCode} · {c.day} {c.start_time}
              </option>
            ))}
          </select>
        </div>
              ...
              return (
                ...
                >
                  <div className="flex items-center justify-between">
                    ...
                    <span
                      className={`text-xs px-2 py-0.5 rounded-full font-medium ${active ? "bg-green-50 text-green-700" : "bg-gray-100 text-gray-500"}`}
                    >
                      {active ? "🟢 Live" : "Closed"}
                    </span>
                  </div>
              ...
              return (
                ...
                >
                  <div className="flex items-start justify-between">
                    ...
                    <span
                      className={`text-xs px-2 py-0.5 rounded-full font-medium shrink-0 ${overdue ? "bg-red-50 text-red-600" : "bg-green-50 text-green-700"}`}
                    >
                      {a.deadline
      ...
      {complaints.map((c: any) => (
        <div key={c.id} className="rounded-3xl border border-gray-100 p-6">
          <div className="flex items-start justify-between mb-3">
            ...
            <span
              className={`text-xs px-2.5 py-1 rounded-full font-medium shrink-0 ${c.status === "resolved" ? "bg-green-50 text-green-700" : "bg-amber-50 text-amber-700"}`}
            >
              {c.status}
            </span>
          </div>
          <p className="text-sm text-gray-600 bg-gray-50 rounded-2xl px-4 py-3 mb-4">
            {c.message}
          </p>
          {c.reply ? (
